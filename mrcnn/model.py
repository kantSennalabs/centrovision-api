...
def load_image_gt(dataset, config, image_id, augmentation=None):
    ...
    # print(mask.shape, len(class_ids))
...
class getCurrentEpoch((keras.callbacks.Callback)):
    def __init__(self, target_epochs):
        self.target_epochs = target_epochs
        
    def current_epoch(self, epoch):
        print_out("Current epoch", epoch)
        print_out(f"Statusxx: Training at {epoch}/{self.target_epochs}")
        print_out(f"Totalxxcomplete: {int(epoch)/int(self.target_epochs)*100}%")
    ...
    def set_log_dir(self, model_path=None):
        ...
        if model_path:
            ...
            if m:
                ...
                print_out('Re-starting from epoch %d' % self.epoch)

        print_out(model_path)
        print_out(self.config.NAME)
